...
# bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/$(uname -m)-$(uname -s)/init_debian.sh")
#-------------------------------------------------------#
 ...
 if [ -s "${SYSTMP}/INITIALIZED" ]; then
    ...
    if [ "$CONTINUE" == "YES" ]; then
         #Install CoreUtils & Deps
          ...
          #Install Build Des
          packages="aria2 automake bc binutils b3sum build-essential ca-certificates ccache diffutils dos2unix gawk git-lfs imagemagick lzip jq libtool libtool-bin make musl musl-dev musl-tools p7zip-full rsync texinfo wget"
          for pkg in $packages; do DEBIAN_FRONTEND="noninteractive" sudo apt install -y --ignore-missing "$pkg"; done
          for pkg in $packages; do DEBIAN_FRONTEND="noninteractive" sudo apt install -y --ignore-missing "$pkg"; done
          packages="autoconf automake autopoint binutils bison build-essential byacc ca-certificates clang flex file jq libtool libtool-bin patch patchelf pkg-config qemu-user-static scons tree wget"
          for pkg in $packages; do DEBIAN_FRONTEND="noninteractive" sudo apt install -y --ignore-missing "$pkg"; done
          for pkg in $packages; do DEBIAN_FRONTEND="noninteractive" sudo apt install -y --ignore-missing "$pkg"; done
    ...
    if [ "$CONTINUE" == "YES" ]; then
         ...
         ##Setup Void (xbps)
          ...
          sudo curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/Baseutils/xbps/xbps-rindex" -o "/usr/local/bin/xbps-rindex" && sudo chmod +x "/usr/local/bin/xbps-rindex"
