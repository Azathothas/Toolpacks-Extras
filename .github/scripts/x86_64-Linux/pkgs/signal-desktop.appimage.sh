#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/signal-desktop.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="signal-desktop.appimage"
export BIN_ID="org.signal.Signal"
export APK_PKG="signal-desktop"
export ARCHLINUX_PKG="signal-desktop"
export REPOLOGY_PKG="signal-desktop"
export SOURCE_URL="https://github.com/karo-solutions/Signal-Desktop-AppImage"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(git ls-remote --tags "${SOURCE_URL}" | awk -F/ '/tags/ && !/{}$/ {print $NF}' | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1| tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "${SOURCE_URL}" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
