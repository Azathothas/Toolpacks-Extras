#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/zoom-us.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
##Main
set -x ; export TZ="UTC"
export SKIP_BUILD="YES" #low effort broken nonsense
#zoom : zoom.us video conferencing application
export BIN="zoom-us.appimage"
export BIN_ID="us.zoom.Zoom"
export ARCHLINUX_PKG="zoom"
export REPOLOGY_PKG="zoom-videoconference"
export SOURCE_URL="https://github.com/probonopd/Zoom.appimage"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(curl -qfsSL "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=zoom" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/probonopd/Zoom.appimage" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
