#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/qbittorrent-enhanced.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#qbittorrent : A qBittorrent for with Enhanced Features
export BIN="qbittorrent-enhanced.appimage"
export BIN_ID="org.qbittorrent.qBittorrent"
export REPOLOGY_PKG="qbittorrent-enhanced"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset ".AppImage" --asset "x86_64"  --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
