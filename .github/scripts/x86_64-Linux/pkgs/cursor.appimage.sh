#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/cursor.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="cursor.appimage"
export BIN_ID="xxx.com.cursor"
export ARCHLINUX_PKG="cursor"
export REPOLOGY_PKG="cursor"
export SOURCE_URL="https://github.com/getcursor/cursor"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       DOWNLOAD_URL="https://downloader.cursor.sh/linux/appImage/x64" && export DOWNLOAD_URL="${DOWNLOAD_URL}"
