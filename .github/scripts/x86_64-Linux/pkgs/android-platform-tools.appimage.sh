#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/android-platform-tools.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#android-platform-tools : Android SDK Platform-Tools (adb, fastboot & more)
export BIN="android-platform-tools.appimage"
export BIN_ID="xxx.com.android.developer"
export ARCHLINUX_PKG="android-sdk-platform-tools"
export REPOLOGY_PKG="android-platform-tools"
export SOURCE_URL="https://android.googlesource.com/platform/tools/base"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="android-platform-tools"
       ...
       timeout 3m eget "https://github.com/Samueru-sama/android-tools-AppImage" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
