#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/kdenlive.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#kdenlive : FOSS video editor, based on MLT Framework and KDE Frameworks
export BIN="kdenlive.appimage"
export BIN_ID="org.kde.kdenlive"
export ARCHLINUX_PKG="kdenlive"
export REPOLOGY_PKG="kdenlive"
export SOURCE_URL="https://invent.kde.org/multimedia/kdenlive"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       DOWNLOAD_URL="$(curl -qfsSL "https://kdenlive.org/en/download/" | grep -o 'href="[^"]*"' | sed 's/href="//' | sed 's/"$//' | grep -Ei "AppImage" | grep -Eiv 'aarch|arm|zsync' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && DOWNLOAD_URL="${DOWNLOAD_URL}"
       RELEASE_TAG="$(echo "${DOWNLOAD_URL}" | grep -o '[0-9]\+\(\.[0-9]\+\)\+' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "${DOWNLOAD_URL}" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
