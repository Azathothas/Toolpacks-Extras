#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/bottles.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#bottles : An easy-to-use wineprefix manager to Run Windows software and games on Linux
export BIN="bottles.appimage"
export BIN_ID="com.usebottles.bottles"
export ARCHLINUX_PKG="bottles"
export REPOLOGY_PKG="bottles"
export SOURCE_URL="https://github.com/bottlesdevs/Bottles"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(curl -qfsSL "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=bottles" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/ivan-hc/Bottles-appimage" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
