#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/deadbeef-nightly.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="deadbeef-nightly.appimage"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="deadbeef-nightly"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="nightly" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/Samueru-sama/DeaDBeeF-AppImage" --asset "Nightly" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
