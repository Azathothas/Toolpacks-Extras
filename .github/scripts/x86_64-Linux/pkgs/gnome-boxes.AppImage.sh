#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/gnome-boxes.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="gnome-boxes.appimage"
export BIN_ID="org.gnome.Boxes"
export ARCHLINUX_PKG="gnome-boxes"
export REPOLOGY_PKG="gnome-boxes"
export SOURCE_URL="https://gitlab.gnome.org/GNOME/gnome-boxes"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="gnome-boxes"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(curl -qfsSL "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=gnome-boxes" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/ivan-hc/Boxes-appimage" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
