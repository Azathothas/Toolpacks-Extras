#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/krita.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="krita.appimage"
export BIN_ID="org.kde.krita"
export ARCHLINUX_PKG="krita"
export REPOLOGY_PKG="krita"
export SOURCE_URL="https://invent.kde.org/graphics/krita"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       DOWNLOAD_URL="$(curl -qfsSL "https://krita.org/en/download/" | grep -iEo 'https://download\.kde\.org/stable/krita/[^"]*\.appimage' | sed 's/"$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export DOWNLOAD_URL="${DOWNLOAD_URL}"
