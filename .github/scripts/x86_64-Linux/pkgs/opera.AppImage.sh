#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/opera.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="opera.appimage"
export BIN_ID="com.opera.Opera"
export ARCHLINUX_PKG="opera"
export REPOLOGY_PKG="opera"
export SOURCE_URL="https://github.com/ivan-hc/Opera-appimage"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(curl -qfsSL "https://api.github.com/repos/ivan-hc/Opera-appimage/releases/tags/continuous" -H "Authorization: Bearer ${GITHUB_TOKEN}" | jq -r '.assets[] | .name' | grep -o '[0-9]\+\(\.[0-9]\+\)\+' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/ivan-hc/Opera-appimage" --asset "x86_64" --asset "stable" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
