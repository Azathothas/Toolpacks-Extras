...
if [ "${SKIP_BUILD}" == "NO" ]; then
    ...
    if [ "${BUILD_NIX_APPIMAGE}" == "YES" ]; then
      ##Create NixAppImage   
       ...
       #pushd "/opt/nix/nix-bundle" >/dev/null 2>&1 && "./nix2appimage.sh" "${NIX_PKGNAME}"
       #find "/opt/nix/nix-bundle" -maxdepth 1 -type f -iregex ".*\.AppImage" | xargs realpath | xargs -I "{}" rsync -achvL --remove-source-files "{}" "${OWD}/${APP}.AppImage" ; popd >/dev/null 2>&1
      #Copy
      ...
      #HouseKeeping
       ...
       #Repack
         if [ -d "${APPIMAGE_EXTRACT}" ] && [ $(du -s "${APPIMAGE_EXTRACT}" | cut -f1) -gt 100 ]; then
          ...
          #Icon
           curl -qfsSL "https://raw.githubusercontent.com/Azathothas/Toolpacks-Extras/refs/heads/main/.github/assets/icons/puddletag.png" -o "${APPIMAGE_EXTRACT}/${APP}.png"
