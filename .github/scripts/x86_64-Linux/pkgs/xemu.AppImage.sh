#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/xemu.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="xemu.appimage"
export BIN_ID="app.xemu.xemu"
export REPOLOGY_PKG="xemu"
export SOURCE_URL="https://github.com/xemu-project/xemu"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset "AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --asset "^dbg" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
