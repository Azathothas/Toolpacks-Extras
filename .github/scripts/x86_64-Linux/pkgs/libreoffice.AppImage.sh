#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/libreoffice.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="libreoffice.appimage"
export BIN_ID="org.libreoffice.LibreOffice"
export ARCHLINUX_PKG="libreoffice"
export REPOLOGY_PKG="libreoffice"
export SOURCE_URL="https://github.com/LibreOffice/core"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="libreoffice"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(git ls-remote --tags "${SOURCE_URL}" | grep -iE 'libreoffice' | grep -Eiv 'alpha|beta|freeze|testing|rc' | awk -F/ '/tags/ && !/{}$/ {print $NF}' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://appimages.libreitalia.org/LibreOffice-fresh.standard-x86_64.appimage" --tag "${RELEASE_TAG}" --asset ".AppImage" --asset "^aarch64" --asset "^arm64" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping
