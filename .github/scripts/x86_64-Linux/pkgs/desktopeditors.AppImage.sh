#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/desktopeditors.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#desktopeditors : Office suite with text, spreadsheet and presentation editors
export BIN="desktopeditors.appimage"
export BIN_ID="org.onlyoffice.desktopeditors"
export ARCHLINUX_PKG="onlyoffice-bin"
export REPOLOGY_PKG="onlyoffice-desktopeditors"
export SOURCE_URL="https://github.com/ONLYOFFICE/DesktopEditors"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset ".AppImage" --asset "^aarch64" --asset "^arm64" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
