#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/microsoft-edge-stable.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="microsoft-edge-stable.appimage"
export BIN_ID="com.microsoft.Edge"
export ARCHLINUX_PKG="microsoft-edge"
export REPOLOGY_PKG="microsoft-edge"
export SOURCE_URL="https://github.com/ivan-hc/MS-Edge-appimage"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="microsoft-edge-stable"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(curl -qfsSL "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-stable/" | grep -o 'href="[^"]*"' | sed 's/href="//' | sed 's/"$//' | grep -o '[0-9]\+\(\.[0-9]\+\)\+' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/ivan-hc/MS-Edge-appimage" --asset "x86_64" --asset "stable" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
