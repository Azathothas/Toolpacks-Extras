#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/dolphin-emu.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="dolphin-emu.appimage"
export BIN_ID="org.DolphinEmu.dolphin-emu"
export ARCHLINUX_PKG="dolphin-emu"
export REPOLOGY_PKG="dolphin-emu"
export SOURCE_URL="https://github.com/dolphin-emu/dolphin"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="dolphin-emu"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(gh release list --repo "https://github.com/qurious-pixel/dolphin" --order "desc" --exclude-drafts --exclude-pre-releases --json "tagName" | jq -r '.[0].tagName | gsub("\\s+"; "")' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/qurious-pixel/dolphin" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
