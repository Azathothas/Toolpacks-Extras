#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/puddletag.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="puddletag.appimage"
export BIN_ID="net.puddletag.puddletag"
export ARCHLINUX_PKG="puddletag"
export REPOLOGY_PKG="puddletag"
export SOURCE_URL="https://github.com/Samueru-sama/puddletag-AppImage"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(gh release list --repo "https://github.com/puddletag/puddletag" --order "desc" --exclude-drafts --exclude-pre-releases --json "tagName" | jq -r '.[0].tagName | gsub("\\s+"; "")' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
