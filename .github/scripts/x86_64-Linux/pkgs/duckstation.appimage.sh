#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/duckstation.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#duckstation : A fast Simulator/Emulator of the Sony PlayStation(TM) Console
export BIN="duckstation.appimage"
export BIN_ID="org.duckstation.DuckStation"
export REPOLOGY_PKG="duckstation"
export SOURCE_URL="https://github.com/stenzek/duckstation"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset "x64" --asset "AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --asset "^SSE2" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
