#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/firefox-dev.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#firefox-dev : A web browser built from Firefox source tree
export BIN="firefox-dev.appimage"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="firefox-dev"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(curl -qfsSL "https://api.github.com/repos/srevinsaju/Firefox-Appimage/releases/tags/firefox-devedition" -H "Authorization: Bearer ${GITHUB_TOKEN}" | jq -r '.assets[] | .name' | grep -o '[0-9]\+\(\.[0-9]\+\)\+' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1 | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/srevinsaju/Firefox-Appimage" --tag "firefox-devedition" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping
