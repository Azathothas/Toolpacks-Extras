#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/qbittorrent.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="qbittorrent.appimage"
export BIN_ID="org.qbittorrent.qBittorrent"
export REPOLOGY_PKG="qbittorrent"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       RELEASE_TAG="$(git ls-remote --tags "${SOURCE_URL}" | grep -iE 'release' | grep -Eiv 'alpha|beta|rc' | awk -F/ '/tags/ && !/{}$/ {print $NF}' | tr -d "[:alpha:]" | sed 's/^[^0-9]*//; s/[^0-9]*$//' | sort --version-sort | tail -n 1| tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       curl -qfsSL "https://sourceforge.net/projects/qbittorrent/files/qbittorrent-appimage/qbittorrent-${RELEASE_TAG}/qbittorrent-${RELEASE_TAG}_$(uname -m).appimage/download" -o "${OWD}/${PKG_NAME}"
      #HouseKeeping 
