#!/usr/bin/env bash
#self source:
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/inkscape.appbundle.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="inkscape.appbundle"
export BIN_ID="org.inkscape.Inkscape"
export ARCHLINUX_PKG="inkscape"
export APK_PKG="inkscape"
export DEB_PKG="inkscape"
export REPOLOGY_PKG="inkscape"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
    ...
    if [ "${BUILD_APPBUNDLE}" == "YES" ]; then
      ##Build AppBundle
       ...
       export APP="inkscape"
       export ALPINE_PKG="inkscape"
       export PKG_NAME="${APP}.dwfs.appbundle"
       RELEASE_TAG="$(curl -qfsSL "https://gitlab.alpinelinux.org/alpine/aports/-/raw/master/community/inkscape/APKBUILD" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
      ##Bundle
       ...
       docker run --privileged --net="host" --name "appbundler-alpine" -e ALPINE_PKG="${ALPINE_PKG}" -e PKG_NAME="${PKG_NAME}" "appbundler-alpine" bash -l -c '
        ...
        #Pack 
         if [ -d "${LOCAL_PATH}" ] && [ $(du -s "${LOCAL_PATH}" | cut -f1) -gt 1000 ] && [ -n "${ALPINE_PKG}" ]; then
           ...
           #https://github.com/xplshn/AppBundleHUB/blob/master/recipes/generic/inkscape.sh
           pelfCreator --maintainer "toolpacks-extra" --name "${ALPINE_PKG}" --pkg-add "${ALPINE_PKG}" --entrypoint "${ALPINE_PKG}.desktop"
