#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/firedragon.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#firedragon : A browser based on Floorp browser
export BIN="firedragon.appimage"
export BIN_ID="org.garudalinux.firedragon"
export ARCHLINUX_PKG="firedragon"
export REPOLOGY_PKG="firedragon"
export SOURCE_URL="https://gitlab.com/garuda-linux/firedragon"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="firedragon"
       export PKG_NAME="${APP}.appimage"
       #https://gitlab.com/garuda-linux/firedragon/builder/-/releases/permalink/latest
       RELEASE_TAG="$(curl -qfsSL "https://gitlab.com/api/v4/projects/55893651/releases/" | jq -r '.[0].tag_name' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       curl -qfsSL "https://gitlab.com/api/v4/projects/55893651/releases/${RELEASE_TAG}" | jq --arg ARCH "$(uname -m)" \
