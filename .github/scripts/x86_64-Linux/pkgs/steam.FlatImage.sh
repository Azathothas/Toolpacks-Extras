#!/usr/bin/env bash
#self source:
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/steam.flatimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="steam.flatimage"
export BIN_ID="com.valvesoftware.Steam"
export ARCHLINUX_PKG="steam"
export REPOLOGY_PKG="steam"
export SOURCE_URL="https://github.com/ValveSoftware/steam-for-linux"
      ...
      ##Create (cachyos FlatImage)
       ...
       pushd "$(mktemp -d)" >/dev/null 2>&1
        ...
        #Build
         ...
         packages="freetype2 gcc-libs glxinfo lib32-freetype2 lib32-gcc-libs lib32-mesa lib32-vulkan-intel lib32-vulkan-radeon mesa pcre vulkan-intel vulkan-radeon vulkan-tools xf86-video-amdgpu xf86-video-intel xorg-server"
         for pkg in $packages; do pacman -Sy "$pkg" --noconfirm ; done
         for pkg in $packages; do pacman -Sy "$pkg" --needed --noconfirm ; done
         pacman steam -Sy --noconfirm
         pacman -Ql steam
