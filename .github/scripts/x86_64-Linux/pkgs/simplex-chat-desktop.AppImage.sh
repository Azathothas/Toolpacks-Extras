#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/simplex-chat-desktop.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#simplex-chat-desktop : Messaging network operating without user identifiers of any kind
export BIN="simplex-chat-desktop.appimage"
export BIN_ID="chat.simplex.simplex"
export ARCHLINUX_PKG="simplex-desktop-appimage"
export REPOLOGY_PKG="simplex-chat-desktop"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="simplex-chat-desktop"
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset ".AppImage" --asset "desktop" --asset "$(uname -m)"  --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
