#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/wps-office.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#wps-office : Office suite, formerly Kingsoft Office
export BIN="wps-office.appimage"
export BIN_ID="com.wps.Office"
export ARCHLINUX_PKG="wps-office"
export REPOLOGY_PKG="wps-office"
export SOURCE_URL="https://github.com/videolan/wps-office"
#-------------------------------------------------------#
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       export APP="wps-office"
       export PKG_NAME="${APP}.appimage"
       RELEASE_TAG="$(curl -qfsSL "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=wps-office" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
       timeout 3m eget "https://github.com/ivan-hc/WPS-Office-appimage" --asset ".AppImage" --asset "^aarch64" --asset "^arm" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
