#!/usr/bin/env bash
#self source:
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/ppsspp.appbundle.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="ppsspp.appbundle"
export BIN_ID="org.ppsspp.PPSSPP"
export APK_PKG="ppsspp"
export ARCHLINUX_PKG="ppsspp"
export REPOLOGY_PKG="ppsspp"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
    ...
    if [ "${BUILD_APPBUNDLE}" == "YES" ]; then
      ##Build AppBundle
       ...
       export APP="ppsspp"
       export ALPINE_PKG="ppsspp"
       export PKG_NAME="${APP}.dwfs.appbundle"
       RELEASE_TAG="$(curl -qfsSL "https://gitlab.alpinelinux.org/alpine/aports/-/raw/master/community/ppsspp/APKBUILD" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
      ##Bundle
       ...
       docker run --privileged --net="host" --name "appbundler-alpine" -e ALPINE_PKG="${ALPINE_PKG}" -e PKG_NAME="${PKG_NAME}" "appbundler-alpine" bash -l -c '
        ...
        #Pack 
         if [ -d "${LOCAL_PATH}" ] && [ $(du -s "${LOCAL_PATH}" | cut -f1) -gt 1000 ] && [ -n "${ALPINE_PKG}" ]; then
           ...
           #https://github.com/xplshn/AppBundleHUB/blob/master/recipes/generic/ppsspp.sh
           pelfCreator --maintainer "toolpacks-extra" --name "${ALPINE_PKG}" --pkg-add "${ALPINE_PKG}" --entrypoint "PPSSPPQt.desktop"
