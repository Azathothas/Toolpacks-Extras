#!/usr/bin/env bash
#self source:
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/xfce4-terminal.appbundle.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#xfce4-terminal : A lightweight and easy to use Terminal Emulator
export BIN="xfce4-terminal.appbundle"
export BIN_ID="org.xfce.terminal"
export REPOLOGY_PKG="xfce4-terminal"
export SOURCE_URL="https://gitlab.xfce.org/apps/xfce4-terminal"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
    ...
    if [ "${BUILD_APPBUNDLE}" == "YES" ]; then
      ##Build AppBundle
       ...
       export APP="xfce4-terminal"
       export ALPINE_PKG="xfce4-terminal"
       export PKG_NAME="${APP}.dwfs.appbundle"
       RELEASE_TAG="$(curl -qfsSL "https://gitlab.alpinelinux.org/alpine/aports/-/raw/master/community/xfce4-terminal/APKBUILD" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
      ##Bundle
       ...
       docker run --privileged --net="host" --name "appbundler-alpine" -e ALPINE_PKG="${ALPINE_PKG}" -e PKG_NAME="${PKG_NAME}" "appbundler-alpine" bash -l -c '
        ...
        #Pack 
         if [ -d "${LOCAL_PATH}" ] && [ $(du -s "${LOCAL_PATH}" | cut -f1) -gt 1000 ] && [ -n "${ALPINE_PKG}" ]; then
           ...
           #https://github.com/xplshn/AppBundleHUB/blob/master/recipes/generic/xfce4-terminal.sh
           pelfCreator --maintainer "toolpacks-extra" --name "${ALPINE_PKG}" --pkg-add "${ALPINE_PKG}" --entrypoint "${ALPINE_PKG}.desktop" -x "usr/bin/${ALPINE_PKG}"
