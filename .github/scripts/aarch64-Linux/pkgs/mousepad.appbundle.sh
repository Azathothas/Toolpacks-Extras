#!/usr/bin/env bash
#self source:
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/mousepad.appbundle.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
#mousepad : An easy-to-use, Fast & Simple Editor
export BIN="mousepad.appbundle"
export BIN_ID="org.xfce.mousepad"
export ARCHLINUX_PKG="mousepad"
export APK_PKG="mousepad"
export REPOLOGY_PKG="mousepad"
export SOURCE_URL="https://gitlab.xfce.org/apps/mousepad"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
    ...
    if [ "${BUILD_APPBUNDLE}" == "YES" ]; then
      ##Build AppBundle
       ...
       export APP="mousepad"
       export ALPINE_PKG="mousepad"
       export PKG_NAME="${APP}.dwfs.appbundle"
       RELEASE_TAG="$(curl -qfsSL "https://gitlab.alpinelinux.org/alpine/aports/-/raw/master/community/mousepad/APKBUILD" | sed -n 's/^pkgver=//p' | tr -d '[:space:]')" && export RELEASE_TAG="${RELEASE_TAG}"
      ##Bundle
       ...
       docker run --privileged --net="host" --name "appbundler-alpine" -e ALPINE_PKG="${ALPINE_PKG}" -e PKG_NAME="${PKG_NAME}" "appbundler-alpine" bash -l -c '
        ...
        #Pack 
         if [ -d "${LOCAL_PATH}" ] && [ $(du -s "${LOCAL_PATH}" | cut -f1) -gt 1000 ] && [ -n "${ALPINE_PKG}" ]; then
           ...
           #https://github.com/xplshn/AppBundleHUB/blob/master/recipes/generic/mousepad.sh
           pelfCreator --maintainer "toolpacks-extra" --name "${ALPINE_PKG}" --pkg-add "${ALPINE_PKG}" --entrypoint "org.xfce.mousepad.desktop" -x "usr/bin/${ALPINE_PKG}"
