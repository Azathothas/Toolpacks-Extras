#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/ryujinx.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export BIN="ryujinx.appimage"
export BIN_ID="org.ryujinx.Ryujinx"
export ARCHLINUX_PKG="ryujinx"
export REPOLOGY_PKG="ryujinx"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset "arm64" --asset "AppImage" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
