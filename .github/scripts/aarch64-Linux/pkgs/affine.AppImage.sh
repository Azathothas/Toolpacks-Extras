#!/usr/bin/env bash
#self source: 
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/pkgs/affine.appimage.sh") 
set +x
#-------------------------------------------------------#
#Sanity Checks
if [ "${BUILD}" != "YES" ] || \
...
export ARCHLINUX_PKG="affine-bin"
export REPOLOGY_PKG="affine-whiteboard"
...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     #-------------------------------------------------------#
      ##Fetch
       ...
       timeout 3m eget "${SOURCE_URL}" --tag "${RELEASE_TAG}" --asset "stable" --asset "linux" --asset "aarch64" --asset "appimage" --asset "^x86" --asset "^amd" --asset "^.zsync" --to "${OWD}/${PKG_NAME}"
      #HouseKeeping 
