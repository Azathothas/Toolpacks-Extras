#!/usr/bin/env bash
##
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/$(uname -m)-$(uname -s)/env.sh")
##

#-------------------------------------------------------#
USER="$(whoami)" && export USER="${USER}"
HOME="$(getent passwd ${USER} | cut -d: -f6)" && export HOME="${HOME}"
HF_REPO_DL="https://huggingface.co/datasets/pkgforge/pkgcache/resolve/main/$(uname -m)-$(uname -s)" && export HF_REPO_DL="${HF_REPO_DL}"
