...
# bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/$(uname -m)-$(uname -s)/build_debian.sh")
#-------------------------------------------------------#

#-------------------------------------------------------#
##ENV:$PATH
 HOST_TRIPLET="$(uname -m)-$(uname -s)" && export HOST_TRIPLET="${HOST_TRIPLET}"
 HF_REPO_DL="https://huggingface.co/datasets/pkgforge/pkgcache/resolve/main/${HOST_TRIPLET}" && export HF_REPO_DL="${HF_REPO_DL}"
 #export NIX_SETUP_MODE="EXPENSIVE"
 source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/env.sh")
#-------------------------------------------------------#
  ...
  TOTAL_RECIPES="${#RECIPES[@]}" && export TOTAL_RECIPES="${TOTAL_RECIPES}" ; echo -e "\n[+] Total RECIPES :: ${TOTAL_RECIPES}\n"
    ...
    pushd "$($TMPDIRS)" >/dev/null 2>&1
    {
     ...
     sed -e '/.*github_pat.*/Id' \
        ...
        grep -viE 'github_pat|ghp_|glpat|hf_|token|access_key_id|secret_access_key|cloudflarestorage' "${TEMP_LOG}" | tee "${LOG_PATH}"
    popd >/dev/null 2>&1
    done
...
pushd "$(mktemp -d)" >/dev/null 2>&1
 source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/hf_git_ops.sh")
 hf_git_ops
popd >/dev/null 2>&1
