...
#
# bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/setup_runimage_archlinux.sh")
##
set -x
#-------------------------------------------------------#

#-------------------------------------------------------#
##setup_runimage_archlinux
setup_runimage_archlinux()
{
 #https://github.com/xplshn/pelf/blob/pelf-ng/assets/AppRun.rootfs-based
   ...
   #Extract
     ...
     if [ -d "${ROOTFS_DIR}" ] && [ $(du -s "${ROOTFS_DIR}" | cut -f1) -gt 100 ]; then
       ...
       echo -e "\n[+] RunImage ROOTFS Setup Failed\n"
       exit 1
     fi
   #AppRun
     curl -qfsSL "https://github.com/Azathothas/Run-wrapper/releases/download/continuous/Run-$(uname -m)-$(uname -s)" -o "${APPDIR}/AppRun" && chmod +x "${APPDIR}/AppRun"
   #Deps for ROOTFS
     mkdir -p "${APPDIR}/usr/bin"
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/bash" -o "${ROOTFS_DIR}/bin/bash"
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/bwrap" -o "${APPDIR}/usr/bin/bwrap"
   ...
   #Static Tools (embed + host)
     ...
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/bwrap" -o "/opt/STATIC_TOOLS/bwrap"
     #sudo rsync -achL "/opt/STATIC_TOOLS/bwrap" "/usr/bin/bwrap" && sudo chmod +x "/usr/bin/bwrap"
     #sudo chmod u+s "/usr/bin/bwrap"
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/dwarfs-tools" -o "/opt/STATIC_TOOLS/dwarfs"
     sudo rsync -achL "/opt/STATIC_TOOLS/dwarfs" "/usr/bin/dwarfs" && sudo chmod +x "/usr/bin/dwarfs"
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/Baseutils/fuse3/fusermount3" -o "/opt/STATIC_TOOLS/fusermount"
     sudo rsync -achL "/opt/STATIC_TOOLS/fusermount" "/usr/bin/fusermount" && sudo chmod +x "/usr/bin/fusermount"
     curl -qfsSL "https://bin.pkgforge.dev/$(uname -m)/dwarfs-tools" -o "/opt/STATIC_TOOLS/mkdwarfs"
...
export -f setup_runimage_archlinux
#-------------------------------------------------------#
