#!/usr/bin/env bash
##
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/hf_toolpacks_devenv.sh")
...
HF_DATASET="pkgforge/pkgcache" && export HF_DATASET="${HF_DATASET}"
#Where it will be cloned
HF_REPO_LOCAL="${HOME}/pkgcache" && export HF_REPO_LOCAL="${HF_REPO_LOCAL}" 
...
hf_build_all()
{
 ...
 bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/${HOST_TRIPLET}/build_debian.sh")
}
...
hf_gen_metadata()
{
  source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/pkgcache/main/.github/scripts/$(uname -m)-$(uname -s)/gen_meta.sh")
}
...
hf_clone_repo()
{
 ...
 #Clone 
 pushd "${HOME}" >/dev/null 2>&1 && git clone --depth="1" --filter="blob:none" --no-checkout "https://huggingface.co/datasets/${HF_DATASET}" && cd "./pkgcache"
 #Set Configs 
  git lfs install
...
hf_cleanup_purge(){
  ...
  find "/tmp" -type d -iname "*pkgcache*" 2>/dev/null -exec rm -rfv {} \; 2>/dev/null
...
hf_help()
{
  ...
  echo -e "\n ---LINKS---\n"
  echo -e "[+] Github: https://github.com/pkgforge/pkgcache"
  echo -e "[+] HF Hub: https://huggingface.co/datasets/${HF_DATASET}"
  echo -e "[+] HF DL: ${HF_REPO_DL}/\${PATH}"
  echo -e "[+] Scripts: https://github.com/pkgforge/pkgcache/tree/main/.github/scripts/${HOST_TRIPLET}"
  echo -e "[+] Self: https://github.com/pkgforge/pkgcache/tree/main/.github/scripts/hf_toolpacks_devenv.sh"
  echo -e "\n"
}
export -f hf_help
if [ $# -gt 0 ]; then
