...
#Chmod +xwr
 ...
 cd "${HF_REPO_PKGDIR}" && find "./" -maxdepth 1 -type f |\
    grep -v -E '\.desktop$|\.DirIcon$|\.jq$|\.png$|\.svg$|\.txt$|\.upx$|\.version$|\.xml$|\.zsync$' |\
...
#BLAKE3SUM
 cd "${HF_REPO_PKGDIR}" && find "./" -maxdepth 1 -type f |\
    grep -v -E '\.desktop$|\.DirIcon$|\.jq$|\.png$|\.svg$|\.txt$|\.upx$|\.version$|\.xml$|\.zsync$' | sort | xargs b3sum |\
...
#SHA256SUM
 cd "${HF_REPO_PKGDIR}" && find "./" -maxdepth 1 -type f |\
    grep -v -E '\.desktop$|\.DirIcon$|\.jq$|\.png$|\.svg$|\.txt$|\.upx$|\.version$|\.xml$|\.zsync$' | sort | xargs sha256sum |\
    jq -R -s 'split("\n") | map(select(length > 0) | split(" +"; "g") |{filename: .[1],sum: .[0]}) | sort_by(.filename)' |\
